...
<template>
  <div classe="container">
    <header classe="p-5 mb-4 mt-4 bg-light rounded-3">
    ...
    <ul class="list-group mt-4">
      <li classe="list-group-item" v-for="tarefa in getTarefasFiltradas()">
        <input @change="evento => tarefa.finalizada = evento.target.checked" :checked="tarefa.finalizada"
          :id="tarefa.titulo" type="checkbox">
        <label :class="{ done: terefa.finalizada }" classe="ms-3" :for="tarefa.titulo">
          {{ tarefa.titulo }}
        </label>
      </li>
    </ul>
